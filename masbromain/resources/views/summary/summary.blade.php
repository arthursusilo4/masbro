...
<body class="bg-[#fff5f5] text-white min-h-screen" style="font-family: 'Poppins', sans-serif;">
    <div class="max-w-sm mx-auto min-h-screen">
        ...
        <div class="px-6 space-y-4">
            <h3 class="text-lg text-center font-bold text-[#f32e24] mb-1">Jabatan Cluster <span
                    class="text-[#f35124]">{{ Auth::user()->cluster_id ? Auth::user()->cluster->name : '-' }}</span></h3>


            {{-- name controller --}}
            <div id="membersList">
                <p class="text-gray-500 text-sm text-center">Tekan Jabatan untuk melihat detail kinerja</p>

                @foreach ($usersByJabatan as $item)
                    <div class="member-card bg-white rounded-lg p-4 shadow text-gray-700 mt-3">
                        <button class="w-full text-left font-semibold text-[#f32e24]"
                            onclick="openPopup(
                                {{ $item['jabatan_id'] }},
                                '{{ $item['jabatan_name'] }}',
                                {{ $item['backchecks'] }},
                                {{ $item['brandings'] }},
                                {{ $item['infokompetitors'] }},
                                {{ $item['aktivitas'] }}
                            )">
                            {{ strtoupper($item['jabatan_name']) }}
                        </button>
                    </div>
                @endforeach
            </div>
            ...
            <!-- Pagination -->
            @if ($usersByJabatan->count() > 5)
                <div class="flex justify-center items-center space-x-2 my-4">
                    @foreach (range(1, ceil($usersByJabatan->count() / 5)) as $page)
                        <button onclick="paginateMembers({{ $page }})"
            ...
            <div id="noResults" class="no-results hidden text-center py-8">
                <div class="text-gray-500">
                    <svg class="mx-auto h-12 w-12 mb-4 opacity-50" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M9.172 16.172a4 4 0 015.656 0M9 12h6m-6-4h6m2 5.291A7.962 7.962 0 0118 12a8 8 0 01-2.009 5.291m-4.982 0A7.962 7.962 0 018 12a8 8 0 012.009-5.291m4.982 0A7.962 7.962 0 0016 12a8 8 0 01-2.009 5.291">
                        </path>
                    </svg>
        ...
        <!-- Enhanced Popup -->
        <div id="userPopup"
            class="popup-overlay hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="popup-content bg-white text-gray-700 rounded-xl p-6 w-80 shadow-2xl relative mx-4">
                <button
                    class="absolute top-3 right-3 text-gray-400 hover:text-gray-600 transition-colors duration-200 w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100"
                    onclick="closePopup()">
                ...
                <div class="space-y-3 mb-6">
                    ...
                    <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                        <span class="text-gray-600">Info Aktivitas:</span>
                        <span id="popupAktivitas" class="font-semibold text-[#f32e24]"></span>
                    </div>
                    <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                ...
                {{-- <a id="popupDetailBtn" href="#"
                    class="block text-center action-gradient text-white rounded-lg py-3 font-semibold transition-colors duration-200 shadow-md ">
                    View Details
            </a> --}}
            </div>
        </div>
        ...
        <script>
            ...
            function openPopup(userId, name, backcheck, branding, aktivitas, infokompetitor) {
                ...
                document.getElementById('popupAktivitas').innerText = aktivitas;
